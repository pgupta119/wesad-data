...
class WESADProcessor:
    ...
    def get_samples(self, data, label):
        stride_length = self.fs_dict['label'] * 5  # sliding length of 5 seconds
        window_len = self.fs_dict['label'] * self.window_in_seconds
        n_windows = int((len(data) - window_len) / stride_length) + 1  # Updated the calculation for sliding window
